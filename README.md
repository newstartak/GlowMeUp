...
완성된 결과물은 현장 대형 스크린과 QR 기반 모바일 홈페이지을 통해 함께 확인할 수 있도록 구성하였습니다.
[https://github.com/newstartak/sketcherXWeb/tree/main](https://github.com/newstartak/GlowMeUpWeb)
...
- **스캐너 리소스 재사용 구조 적용**  
  스캐너를 매번 완전히 종료하고 다시 생성하는 방식 대신,  
  백그라운드에 유지한 뒤 필요한 시점에 다시 활성화하는 구조로 설계하여  
  초기화 시간을 줄이고 전체 체험 흐름이 끊기지 않도록 최적화하였습니다.

- **비동기 처리 기반 사용자 경험 개선**  
  프로그램 초기화, 이미지 처리, 서버 통신 등 시간이 걸리는 작업에 비동기 로직을 적용하여  
  메인 흐름이 멈추지 않도록 구성하였습니다.

- **키오스크 환경 대응 입력 UX 구현**  
  물리 키보드 없이도 독립적으로 동작할 수 있도록 가상 키보드를 구현하여  
  전시 및 체험형 환경에 적합한 입력 방식을 구성하였습니다.
...
현장에서 태블릿과 스캐너를 함께 배치하여 사용자가 직접 그림을 그리고,  
스캔 후 AI 결과를 확인할 수 있도록 운영한 모습입니다.
...
| 화면 | 설명 |
|---|---|
| 메인 화면 | 프로그램 단계 및 체험 흐름 안내 |
| 카메라 화면 | 사용자가 그린 원본 이미지를 스캐너로 확인 및 촬영 |
| 키보드 입력 화면 | 가상 키보드를 통한 닉네임 입력 |
| 결과 안내 화면 | 결과 확인 및 QR 기반 모바일 진입 유도 |

---

## 사용자 흐름

1. 메인 화면에서 프로그램 진행 단계 확인
2. 사용자가 그린 그림을 스캐너로 촬영
3. 가상 키보드로 닉네임 입력
4. 촬영 이미지를 서버로 전송
5. AI 변환 처리
6. 키오스크 및 QR을 통해 결과 확인

---

## 프로젝트 의의

이 프로젝트는 사용자가 직접 그린 손그림이라는 아날로그 입력을  
AI 기반의 디지털 결과물로 연결하는 체험형 콘텐츠입니다.

단순 이미지 변환 기능을 넘어서,  
스캐너 연동, 입력 UX, 서버 전송, 키오스크 안내 흐름, 현장 운영까지 포함한  
완성형 인터랙티브 체험 시스템으로 구성한 점이 핵심입니다.
